...
if (isset($_GET['id'])) {
    ...
    // Fetch auxiliary leader details
    $sql = "SELECT * FROM auxiliary_leaders WHERE id='$id'";
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $phone = $_POST['phone'];
        $password = $_POST['password'] ? password_hash($_POST['password'], PASSWORD_DEFAULT) : $leader['password'];
        $auxiliary = $_POST['auxiliary'];

        $sql = "UPDATE auxiliary_leaders SET 
                username='$username', 
                phone='$phone', 
                password='$password', 
                auxiliary='$auxiliary' 
        ...
        if ($conn->query($sql) === TRUE) {
            echo "<script>alert('Auxiliary leader updated successfully.'); window.location.href='view_auxiliary_leaders.php';</script>";
        } else {
            echo "Error: " . $conn->error;
        }
    }
} else {
    echo "<script>alert('Invalid request.'); window.location.href='view_auxiliary_leaders.php';</script>";
}
...
<header class="bg-white shadow-md p-4">
    <div class="container mx-auto flex justify-between items-center gap-100%">
        <h1 class="text-2xl font-semibold">Edit Auxiliary Leader Details</h1>
...
<div class="flex flex-1">
    ...
    <main class="flex-1 p-6 bg-white">
        ...
        <div class="col-span-2">
            ...
            <input type="text" id="username" name="username" value="<?php echo $leader['username']; ?>" required class="w-full p-2 border border-gray-300 rounded mt-1 hover-3d"> 
        </div>
        <div class="col-span-2">
            <label for="phone" class="block text-gray-700">Phone:</label>
            <input type="text" id="phone" name="phone" value="<?php echo $leader['phone']; ?>" required class="w-full p-2 border border-gray-300 rounded mt-1 hover-3d">
        </div>
        <div class="col-span-2">
            <label for="password" class="block text-gray-700">Password:</label>
            <input type="password" id="password" name="password" class="w-full p-2 border border-gray-300 rounded mt-1 hover-3d">
        </div>
        <div class="col-span-2">
            ...
            <select id="auxiliary" name="auxiliary" required class="w-full p-2 border border-gray-300 rounded mt-1 hover-3d">
                <option value="men" <?php if($leader['auxiliary'] == 'men') echo 'selected'; ?>>Men</option>
                <option value="women" <?php if($leader['auxiliary'] == 'women') echo 'selected'; ?>>Women</option>
                <option value="youth" <?php if($leader['auxiliary'] == 'youth') echo 'selected'; ?>>Youth</option>
            </select>
        </div>
        <div class="col-span-2 flex justify-center mt-5">
            <input type="submit" value="Update Leader" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 hover-3d">
        </div>
        </div>
        </form>
    </main>
